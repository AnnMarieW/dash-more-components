...
consumption (with a GPS on a mobile device for example). If `False` (the default value), the device can save resources by 
responding more quickly and/or using less power.  Note: When selected, timeout should be set to a high number or a max age 
should be provided as GPS sometimes takes longer to update.
